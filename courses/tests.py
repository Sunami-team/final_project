...
class CourseListCreateTest(TestCase):
    def setUp(self):
        self.it_manager = User.objects.create_user(username='it_manager', password='test123', email="123@yahoo.com", user_type="it_manager")
        self.deputy_educational = User.objects.create_user(username='deputy_edu', password='test456', email="321@yahoo.com", user_type="deputy_educational")
        self.college = Faculty.objects.create(name="fake_college")
        self.object = Course.objects.create(college=self.college, name="new_course", course_unit="3", course_type="O")  # Create your test object
