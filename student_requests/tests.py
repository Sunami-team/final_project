...
class TestDropTerm(TestCase):
    def setUp(self):
        ...
        self.fake_study_field = StudyField.objects.create(name='ie-chizi', total_units=140, level='کارشناسی', faculty=self.fake_collage)
        self.fake_deputy_educational = DeputyEducational.objects.create(username='us_musa', password='sinasina123',
                                                                        college=self.fake_collage, study_field=self.fake_study_field)
    ...
    def test_drop_term_request_detail_response_200(self):
        url = reverse('student_requests:remove-term-detail', kwargs={'student_id':self.fake_student.pk, 'term_id':self.fake_term.pk})
    ...
    def test_drop_term_request_detail_response_403(self):
        url = reverse('student_requests:remove-term-detail', kwargs={'student_id':self.fake_student.id, 'term_id':self.fake_term.id})
        self.client.force_authenticate(self.user)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 403)
...
class TestAssistantGradeCorrection(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create(username='ya-adame-alaki', password='sinasina!@#')
        self.fake_collage = Faculty.objects.create(name='alaki-hala')
        self.fake_study_field = StudyField.objects.create(name='ie-chizi', total_units=140, level='Bachelor', faculty=self.fake_collage)
    ...
    def test_assistant_grade_correction_list_status_200(self):
        url = reverse('student_requests:assistant-change-grade-list', kwargs={'professor_id':self.fake_professor.id, 'course_id':self.fake_courese_term.id})
    ...
    def test_assistant_grade_correction_student_detail_status_200(self):
        url = reverse('student_requests:assistant-change-grade-list-student-detail', kwargs={'professor_id':self.fake_professor.id, 'course_id':self.fake_courese_term.id, 'student_id':self.fake_student.id})
        self.client.force_authenticate(user=self.fake_deputy_educational)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_assistant_grade_correction_student_put_200(self):
        ...
        self.client.force_authenticate(user=self.fake_deputy_educational)
        data = {
            'response_text': 'Some text from Educational Deputy',
            'approve': True
        }
