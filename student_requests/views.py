...
from .serializers import (
    ...
    CorrectionShowSerializer,
    EmergencyDropRequestSerializer,
    MilitaryServiceRequestSerializer,
...
class TermRemovalRequestViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, pk):
        try:
            ...
            return Response(
                {ـ("error"): "درخواست حذف ترم مورد نظر یافت نشد."},
                status=status.HTTP_404_NOT_FOUND,
            )

        serializer = TermRemovalRequestSerializer(
            term_removal_request, data=request.data, partial=True
        )
    ...
    def retrieve(self, request, pk):
        try:
            ...
            return Response(
                {_("error"): "درخواست حذف ترم مورد نظر یافت نشد."},
                status=status.HTTP_404_NOT_FOUND,
            )
...
class AssistantRemoveTermStudentDetail(APIView):
    ...
    def put(self, request, term_id, student_id):
        ...
        studnet_term_drop = TermDropRequest.objects.get(
            student=student, term=current_term
        )
        serializer = self.serializer_class(
            instance=studnet_term_drop, data=request.data
        )
        ...
        if accept:
            ...
            try:
                send_email.delay(
                    student.email,
                    f"{student.first_name} {student.last_name} ,Your Term Removed",
                )
...
class EmergencyDropRequestView(generics.GenericAPIView):
    ...
    def get_queryset(self):
        ...
        return EmergencyDropRequest.objects.filter(
            student_id=student_id, course__course_id=course_id
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            existing_request = EmergencyDropRequest.objects.filter(
                student=student, course=course_term
            ).first()
            if existing_request:
                return Response(
    ...
    def patch(self, request, *args, **kwargs):
        ...
        try:
            ...
            emergency_drop_request = EmergencyDropRequest.objects.filter(
                student_id=student_id, course__course_id=course_id
            ).first()
            ...
            serializer = self.get_serializer(
                emergency_drop_request, data=request.data, partial=True
            )
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data)
    ...
    def get_object(self):
        ...
        try:
            emergency_request = get_object_or_404(
                EmergencyDropRequest, student_id=student_id
            )
        except NotFound:
...
class AssistantGradeReconsiderationRequestList(APIView):
    ...
    def get(self, request, professor_id, course_id):
        try:
            all_requets = GradeReconsiderationRequest.objects.filter(
                course=course_id, course__professor=professor_id
            )
            serializer = AssistantGradeReconsiderationRequestSerializer(
                all_requets, many=True
            )
            return Response(serializer.data, status=status.HTTP_200_OK)
        except GradeReconsiderationRequest.DoesNotExist:
            return Response(
                {_("errors"): _("Request Does not exist")},
                status=status.HTTP_404_NOT_FOUND,
            )
...
class AssistantGradeReconsiderationRequestStudentDetail(generics.GenericAPIView):
    ...
    def get(self, request, professor_id, course_id, student_id):
        try:
            requet_detail = GradeReconsiderationRequest.objects.get(
                course=course_id, course__professor=professor_id, student=student_id
            )
    ...
    def put(self, request, professor_id, course_id, student_id):
        ...
        serializer = AssistantGradeReconsiderationRequestSerializer(
            instance=requet_detail, data=request.data
        )
        serializer.is_valid(raise_exception=True)
        serializer.save()
        if serializer.validated_data["approve"]:
            try:
                ...
                student_course = StudentCourse.objects.get(
                    student=student_id, course_term=course_id, term=course_term.term
                )
                student_course.course_status = "pass"
                student_course.save()
            except StudentCourse.DoesNotExist:
                return Response(
                    {_("details"): _("Studet Course Does Not exist")},
                    status=status.HTTP_200_OK,
                )
        else:
            return Response(
                {_("details"): _("Request Failed")}, status=status.HTTP_200_OK
            )
        return Response(
            {_("details"): _("Request Accepted")}, status=status.HTTP_200_OK
        )
        ...
        for course_to_add in serializer.validated_data["courses_to_add"]:
            if course_to_add in serializer.validated_data["courses_to_drop"]:
                return Response(
                    {
                        _("detail"): _(
                            "You can not add and drop a course in same time !"
                        )
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class DetailCorrectionRequestByStudent(APIView):
    ...
    def get(self, request, pk):
        correction_requests = CourseCorrectionStudentRequest.objects.filter(
            student__id=pk
        )
...
class CorrectionShowErrors(APIView):
    ...
    def get(self, request, pk, term_id):
        ...
        current_term = Term.objects.get(id=term_id)
        all_current_courses = StudentCourse.objects.filter(
            student=student, term=current_term
        )
        all_passed_courses = StudentCourse.objects.filter(student=student).exclude(
            id__in=all_current_courses
        )
        ...
        for add_course in correction_student.courses_to_add.all():
            ...
            for pre_requisite in add_course.course.courses_required.all():
                if pre_requisite not in all_passed_courses:
                    add_errors[add_course.course.name].append(
                        f"You Did not passed {pre_requisite.name}"
                    )
            ...
            if (
                add_course.capacity
                == StudentCourse.objects.filter(course_term=add_course.course).count()
            ):
            ...
            for current_course in all_current_courses:
                if add_course.class_time == current_course.course_term.class_time:
                    add_errors[add_course.course.name].append(
                        "Course Interference time"
                    )
                if (
                    add_course.exam_date_time
                    == current_course.course_term.exam_date_time
                ):
        ...
        for drop_course in correction_student.courses_to_drop.all():
            ...
            for studnet_course in all_current_courses:
                if studnet_course.course != drop_course.course:
                    for (
                        co_requisite
                    ) in studnet_course.course_term.course.co_requisites.all():
                        if co_requisite == drop_course:
                            drop_errors[drop_course.course.name].append(
                                "this Course have Co Requisite"
                            )
        ...
        return Response(
            {
                ...
                _("status"): correction_student.approval_status,
            },
            status=status.HTTP_200_OK,
        )
    ...
    def post(self, request, pk):
        try:
            with transaction.atomic():
                student = Student.objects.get(id=pk)
                correction_student = CourseCorrectionStudentRequest.objects.get(
                    student=student
                )
                if correction_student.approval_status:
                    final_correction = (
                        CourseCorrectionStudentSendToAssistant.objects.create(
                    ...
                    return Response(
                        _("add to Correction Submit"), status=status.HTTP_200_OK
                    )
                else:
                    raise serializers.ValidationError(
                        _("Add and drop Corses does not correct")
                    )
...
class CorrectionSendForm(APIView):
    def get(self, request, pk, term_id):
        student = Student.objects.get(id=pk)
        correction_student = CourseCorrectionStudentSendToAssistant.objects.get(
            student=student
        )
        try:
        ...
        if not correction_student.courses_to_add.all():
            return Response(
                _("Corses Corrections is Empty"), status=status.HTTP_400_BAD_REQUEST
            )
...
class DetailSelectionRequestByStudent(APIView):
    ...
    def get(self, request, pk):
        selection_requests = CourseSelectionStudentRequest.objects.filter(
            student__id=pk
        )
...
class SelectionSubmit(APIView):
    ...
    def post(self, request, pk):
        try:
            with transaction.atomic():
                student = Student.objects.get(id=pk)
                selection_student = CourseSelectionStudentRequest.objects.get(
                    student=student
                )
                if selection_student.approval_status:
                    final_selection = (
                        CourseSelectionStudentSendToAssistant.objects.create(
...
class SelectionSendForm(APIView):
    def get(self, request, pk, term_id):
        student = Student.objects.get(id=pk)
        selection_student = CourseSelectionStudentSendToAssistant.objects.get(
            student=student
        )
        try:
            selected_term = Term.objects.get(id=term_id)
...
class GradeReconsiderationRequestView(generics.GenericAPIView):
    ...
    def get(self, request, course_id, student_id):
        # student = Student.objects.get(id=student_id)
        try:
            data = GradeReconsiderationRequest.objects.get(
                course__course=course_id, student=student_id
            )
            ...
            return Response(
                _("Request Does Not Exists!"), status=status.HTTP_404_NOT_FOUND
            )

    def post(self, request, course_id, student_id):
        data = GradeReconsiderationRequest.objects.filter(
            course__course=course_id, student=student_id
        ).exists()
        if not data:
            try:
                student_course = StudentCourse.objects.get(
                    student=student_id, course_term=course_id
                )
            except StudentCourse.DoesNotExist:
                return Response(
                    _("Object Does Not Exists!"), status=status.HTTP_404_NOT_FOUND
                )
            ...
            if grade:
                ...
                serializer.save()
                return Response(
                    {_("details"): _("request created successfully")},
                    status=status.HTTP_201_CREATED,
                )
            else:
                return Response(
                    _("Grade Does Not Exists!"), status=status.HTTP_404_NOT_FOUND
                )
        else:
            return Response(
                _("Your request is filled!!"), status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def put(self, request, course_id, student_id):
        try:
            ...
            serializer.save()
            return Response(
                {_("details"): _("request modified successfully")},
                status=status.HTTP_200_OK,
            )
        except GradeReconsiderationRequest.DoesNotExist:
            return Response(
